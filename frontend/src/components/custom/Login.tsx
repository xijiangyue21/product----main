...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError('');
    if (!email || !password) { setError('请填写邮筱和密码'); return; }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-[#080C10] flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-[#0F1620] border border-[#1A2A3A] rounded-2xl p-8">
          <h2 className="text-xl font-bold text-[#E8F0F8] mb-6">登录账户</h2>
          ...
          <div className="mt-6 text-center">
            <span className="text-sm text-[#5A7A9A] font-mono">还没有账户？ </span>
            <button
